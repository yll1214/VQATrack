...
class OTBDataset(BaseDataset):
    ...
    def __init__(self, use_llava_description=True):
        ...
        # 确保语言描述目录存在
        self.language_dir = os.path.join(self.base_path, 'llava_模板+图像')
    ...
    def _construct_sequence(self, sequence_info):
        ...
        if self.use_llava_description and LLAVA_AVAILABLE and os.path.exists(first_frame_path):
            try:
        ...
        print(language)
        print("========================")
        # 关键：返回原始的边界框数据，语言描述只是附加信息
        return Sequence(
            sequence_info['name'], 
            frames, 
            'otb', 
            ground_truth_rect[init_omit:,:],  # 原始边界框数据，保持不变
            object_class=sequence_info['object_class'], 
            language=language  # 语言描述（可能是LLaVA生成或原始的）
        )
