...
# 将llava.py所在目录添加到Python路径（与OTBDataset一致）
...
class LaSOTDataset(BaseDataset):
    ...
    def _draw_bbox_on_image(self, image_path, bbox):
        ...
        try:
            ...
            print(f"[LaSOTDataset] 绘制边界框失败: {e}")
    ...
    def _construct_sequence(self, sequence_name):
        ...
        # 使用LLaVA生成的语言文件路径
        language_file_llava = os.path.join(self.base_path, class_name, sequence_name, "nlp_模板+标注.txt")
    ...
    def _generate_all_language_descriptions(self):
        """一次性生成所有序列的语言描述（使用模板+标注）"""
        for i, sequence_name in enumerate(self.sequence_list):
            class_name = sequence_name.split('-')[0]
            
            # 检查是否已经生成过
            language_file_llava = os.path.join(self.base_path, class_name, sequence_name, "llava_模板+标注.txt")
            if os.path.exists(language_file_llava):
                print(f"[{i+1}/{len(self.sequence_list)}] 跳过已生成的: {sequence_name}")
                continue
                
            try:
                print(f"\n[{i+1}/{len(self.sequence_list)}] 处理序列: {sequence_name}")
                ...
                if not os.path.exists(anno_path) or not os.path.exists(frames_path):
                ...
                # 第一帧图片路径
                first_frame_path = os.path.join(frames_path, "00000001.jpg")
                ...
                annotated_path = os.path.join(seq_save_dir, "annotated_frame_00000001.jpg")
                annotated_image.save(annotated_path, 'JPEG')
                print(f"  标注图像已保存: {annotated_path}")
                
                # 3. 使用带标注的图像调用LLaVA（与OTBDataset相同）
                ...
                print(language)
                print("========================")
                # 5. 保存语言描述到文件（nlp_llava.txt）
                os.makedirs(os.path.dirname(language_file_llava), exist_ok=True)
                with open(language_file_llava, 'w') as f:
                    f.write(language)
                    
            except Exception as e:
                print(f"  处理 {sequence_name} 时出错: {e}")
                # 如果失败，使用原始描述
                original_language_file = os.path.join(self.base_path, class_name, sequence_name, "nlp.txt")
                if os.path.exists(original_language_file):
                    with open(original_language_file, 'r') as f:
                        language = f.readlines()[0].rstrip()
                    with open(language_file_llava, 'w') as f:
                        f.write(language)
