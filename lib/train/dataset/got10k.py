...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        #print("Class list before sorting:", self.class_list)
        #print("None values in list:", sum(1 for x in self.class_list if x is None))
        for idx, class_name in enumerate(self.class_list):
            if class_name is None:
                print(f"Warning: None value found at index {idx}, sequence: {self.sequence_list[idx]}")
        self.class_list = [x for x in self.class_list if x is not None]
        self.sequence_list = [seq for seq, cls in zip(self.sequence_list, self.class_list) if cls is not None]
        self.class_list.sort()  
