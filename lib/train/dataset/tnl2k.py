...
class TNL2K(BaseVideoDataset):
    ...
    def _read_language(self, seq_path):
        language = os.path.join(seq_path, "language.txt")
        try:
            with open(language, 'r') as f:
                language = f.readlines()
            if not language:
                raise ValueError(f"No language data found in {language}")
            return language[0].rstrip()
        except Exception as e:
            print(f"Error reading language from {seq_path}: {e}")
    ...
    def _get_frame(self, seq_path, frame_id):
        ...
        if frame_id >= len(images):
            frame_id = len(images) - 1
        #return self.image_loader(os.path.join(seq_path, 'imgs', images[frame_id]))
        return self.image_loader(images[frame_id])
