...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None, return_bbox=False):
    ...
    参数:
        im - OpenCV图像
        target_bb - 目标边界框 [x, y, w, h]
        search_area_factor - 裁剪区域与目标区域的尺寸比例
        output_sz - (可选)输出尺寸(正方形)。如果为None则不调整大小
    
    返回:
        根据不同的参数组合返回不同的值，可能包含：
        - 裁剪后的图像
        - 调整尺寸的比例因子
        - 注意力掩码
        - 分割掩码(如果提供)
    ...
    if not isinstance(target_bb, list):
        ...
        x, y, w, h = target_bb  
    # 计算裁剪区域尺寸 = sqrt(目标面积) × 搜索因子
    crop_sz = math.ceil(math.sqrt(w * h) * search_area_factor)
    # 有效性检查：裁剪尺寸不能小于1
    if crop_sz < 1:
        raise Exception('目标边界框过小')
    # 计算裁剪区域坐标 --------------------------------------------------
    x1 = int(round(x + 0.5 * w - crop_sz * 0.5))  # 左边界
    x2 = int(x1 + crop_sz)                        # 右边界
    y1 = int(round(y + 0.5 * h - crop_sz * 0.5))  # 上边界
    y2 = int(y1 + crop_sz)                        # 下边界
    # 计算需要的填充量(当裁剪区域超出图像边界时) --------------------------
    x1_pad = int(max(0, -x1))              # 左侧填充量
    x2_pad = int(max(x2 - im.shape[1] + 1, 0))  # 右侧填充量
    y1_pad = int(max(0, -y1))              # 上方填充量
    y2_pad = int(max(y2 - im.shape[0] + 1, 0))  # 下方填充量
    # 执行图像裁剪(考虑填充调整) ----------------------------------------
    ...
    # 创建注意力掩码(1表示填充区域，0表示有效区域) -----------------------
    H, W, _ = im_crop_padded.shape
    att_mask = np.ones((H,W))  # 初始化为全1(表示都是填充区域)
    # 确定有效区域边界
    end_x, end_y = -x2_pad, -y2_pad  # 默认值
    if y2_pad == 0: end_y = None     # 无下方填充
    if x2_pad == 0: end_x = None     # 无右侧填充
    # 将有效区域置为0
    att_mask[y1_pad:end_y, x1_pad:end_x] = 0
    # 如果有掩码则进行填充
    if mask is not None:
    ...
    # 根据参数组合返回不同的结果 ========================================
    if return_bbox:  # 如果需要返回边界框
        if output_sz is not None:  # 如果需要调整尺寸
            resize_factor = output_sz / crop_sz  # 计算缩放因子
            # 调整图像和注意力掩码尺寸
        ...
        if output_sz is not None:  # 需要调整尺寸
            ...
            if mask is None:
                # 情况7: 无掩码 → 返回3个值(比例因子为1.0)
                return im_crop_padded, att_mask.astype(np.bool_), 1.0
            else:
                # 情况8: 有掩码 → 返回4个值(比例因子为1.0)
