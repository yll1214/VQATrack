...
def RandomHorizontalFlip(im, phrase, box):
    ...
    if random.random() < p:
        ...
        phrase[phrase == 2187] = -100 #for bert
        phrase[phrase == 2157] = 2187
        phrase[phrase == -100] = 2157
        # phrase[phrase == 1823] = -100 #for clip
        # phrase[phrase == 1155] = 1823
        # phrase[phrase == -100] = 1155
...
def grounding_resize(im, output_sz, bbox, phrase, mask=None, aug_translate=True, center_place=False):
    ...
    # img1 = cv2.rectangle(deepcopy(im), (int(x1), int(y1)), (int(x2), int(y2)), (255, 0, 0), 1)
    # plt.imshow(img1)
    # plt.show()
    ...
    if output_sz == 384:
        min_size, max_size = 256, 360
        # sizes1 = [240, 300, 360]
        # sizes2 = [256, 288, 320, 352, 384]
        sizes1 = [output_sz - 16 * i for i in range(output_sz // 48)]
        sizes2 = [output_sz - 32 * i for i in range(1, output_sz // 64 - 1)]
    elif output_sz == 256:
        min_size, max_size = 186, 240
        sizes1 = [180, 210, 240]
        sizes2 = [186, 192, 208, 224, 240]
    elif output_sz == 320:
        min_size, max_size = 180,300
        sizes1 = [172,236,300]
        sizes2 = [180,210,240,270,300]
    ...
    if done:
        if random.random() < p:
            ...
            im, box = RandomSizeCrop(im, box, min_size=min_size, max_size=max_size, max_cnt=20, check_method=dict(func='iou', iou_thres=0.9))
            im, box = RandomResize(sizes1, im, box, resize_long_side=True)
    ...
    # plt.show()
    # x11, y11, x21, y21 = box[0], box[1], box[2], box[3]
    # img2 = cv2.rectangle(deepcopy(img), (int(x11), int(y11)), (int(x21), int(y21)), (255, 0, 0), 1)
    # # #plt.imsave("debug/image.png", img2)
    # plt.imshow(img2)
    # plt.show()
