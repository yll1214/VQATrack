...
class Lasot(BaseVideoDataset):
    ...
    def _read_bb_anno(self, seq_path):
        ...
        try:
            # 先以字符串形式读取，然后清理数据
            df = pandas.read_csv(bb_anno_file, delimiter=',', header=None, dtype=str, na_filter=False)
            
            # 清理数据：移除非法字符，转换格式
            df = df.replace('[^0-9.eE-]', '', regex=True)  # 移除非数字字符
            df = df.apply(pandas.to_numeric, errors='coerce')  # 尝试转换为数值
            
            # 检查并处理无效值
            if df.isnull().values.any():
                print(f"警告: 标注文件 {bb_anno_file} 包含无效数据，已自动填充为0")
                df = df.fillna(0)
            ...
            print(f"无法读取标注文件 {bb_anno_file}: {str(e)}")
            return torch.zeros((1, 4), dtype=torch.float32)  # 假设是4列边界框数据
