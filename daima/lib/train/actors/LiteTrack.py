...
@registry.ACTORS.register("LiteTrack")
class LiteTrackActor(BaseActor):
    ...
    def cont_gt(self, gt_bboxes, size):
        gt_bboxes = gt_bboxes.clone().detach() 
        gt_bboxes.requires_grad_(False) 
        new_bboxes = torch.zeros_like(gt_bboxes)
        new_bboxes[:, :2] = torch.clamp(gt_bboxes[:, :2], min=0.0, max=1.0)  # x1,y1
        new_bboxes[:, 2:] = torch.clamp(gt_bboxes[:, 2:], min=0.01, max=1.0)  # w,h
        new_bboxes[:, 2] = torch.min(new_bboxes[:, 2], 1.0 - new_bboxes[:, 0])  # x2
        new_bboxes[:, 3] = torch.min(new_bboxes[:, 3], 1.0 - new_bboxes[:, 1])  # y2
        gt_bboxes = new_bboxes 
        bboxes = box_cxcywh_to_xyxy(box_xywh_to_cxcywh_scale(gt_bboxes, self.cfg.TRAIN.CTR_RATIO)) * size
        cood = torch.linspace(0.5, size-0.5, size, device=gt_bboxes.device).unsqueeze(0).repeat(gt_bboxes.shape[0], 1)
        x_mask = ((cood > bboxes[:, 0:1]) & (cood < bboxes[:, 2:3])).unsqueeze(1)
        y_mask = ((cood > bboxes[:, 1:2]) & (cood < bboxes[:, 3:4])).unsqueeze(2)
        mask_c = (x_mask & y_mask)
        cx = torch.clamp(((bboxes[:, 0] + bboxes[:, 2]) / 2).floor().long(), 0, size-1)
        cy = torch.clamp(((bboxes[:, 1] + bboxes[:, 3]) / 2).floor().long(), 0, size-1)
        bid = torch.arange(len(cx), device=cx.device)
    ...
    def anno2mask(self, gt_bboxes, size, reverse=False):
        ...
        gt_bboxes.requires_grad_(False) 
        new_bboxes = torch.zeros_like(gt_bboxes)
        new_bboxes[:, :2] = torch.clamp(gt_bboxes[:, :2], min=0.0, max=1.0)
        new_bboxes[:, 2:] = torch.clamp(gt_bboxes[:, 2:], min=0.01, max=1.0)
        new_bboxes[:, 2] = torch.min(new_bboxes[:, 2], 1.0 - new_bboxes[:, 0])
        new_bboxes[:, 3] = torch.min(new_bboxes[:, 3], 1.0 - new_bboxes[:, 1])
        gt_bboxes = new_bboxes
        bboxes = box_xywh_to_xyxy(gt_bboxes) * size
        cood = torch.arange(size, device=gt_bboxes.device).unsqueeze(0).repeat(gt_bboxes.shape[0], 1) + 0.5
        x_mask = ((cood > bboxes[:, 0:1]) & (cood < bboxes[:, 2:3])).unsqueeze(1)
        y_mask = ((cood > bboxes[:, 1:2]) & (cood < bboxes[:, 3:4])).unsqueeze(2)
        mask = (x_mask & y_mask)
        cx = torch.clamp(((bboxes[:, 0] + bboxes[:, 2]) / 2).floor().long(), 0, size-1)
        ...
        if reverse:
            half = bid.shape[0] // 2
            mask = torch.cat([mask[half:], mask[:half]], dim=0)
